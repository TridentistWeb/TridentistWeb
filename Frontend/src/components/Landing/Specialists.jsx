    ...
    const fetchDoctors = async () => {
      try {
        ...
        const enriched = res.data.map((doc) => ({
          ...
          videoUrl: doc.videoUrl || '/' + doc.nombres.trim().split(' ')[0].toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '') + '_' + doc.apellidos.trim().split(' ')[0].toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '') + '.webm',
