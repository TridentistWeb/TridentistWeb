...
import com.utp.tridentist.service.DoctorService;
...
public class DoctorController {
    ...
    public ResponseEntity<Doctor> getById(@PathVariable Integer id) {
        Optional<Doctor> entity = service.findById(id);
        return entity.map(ResponseEntity::ok)
                     .orElseGet(() -> ResponseEntity.notFound().build());
    }

    @PostMapping
    public Doctor create(@RequestBody Doctor entity) {
        return service.save(entity);
    }

    @PutMapping("/{id}")
    public ResponseEntity<Doctor> update(@PathVariable Integer id, @RequestBody Doctor entityDetails) {
